...
class ResetPasswordRequestToken(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # user = User.objects.fillter(**{'{}__iexact'.format(get_password_reset_lookup_field()): email})
    ...
    #
        ...
        if not token.key:
            # no token exists, generate a new token
            token.user_agent=request.META.get(HTTP_USER_AGENT_HEADER, '')
            token.ip_address=request.META.get(HTTP_IP_ADDRESS_HEADER, '')
            token.save()
